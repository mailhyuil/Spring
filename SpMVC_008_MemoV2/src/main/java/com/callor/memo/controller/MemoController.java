...
public class MemoController {
	...
	public String delete(@PathVariable("seq") String seq) {
		...
		return "redirect:/memo/trashcan";
	}
	
	@RequestMapping(value="/trashcan",method=RequestMethod.GET)
	public String trashcan(Model model, HttpSession httpSession) {
		String username = (String) httpSession.getAttribute("USERNAME");
		
		if(username != null) {
			List<MemoDTO> memos = memoService.findTrashcanByAuthor(username);
			model.addAttribute("MEMOS", memos);
		}
		return "trashcan";
	}
	
	@RequestMapping(value="/{seq}/toTrashcan",method=RequestMethod.GET)
	public String toTrashcan(@PathVariable("seq") String seq) {
		MemoDTO memoDTO = memoService.findById(Long.valueOf(seq));
		memoDTO.setM_status(1);
