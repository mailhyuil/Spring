...
public class NaverParent<VO> {
	...
	/*
	 * Naver API 를 사용하여 News, Movies, Books 정보를 가져오려고 한다
	 * 이때 RestTemplate 를 사용하여 데이터를 가져오는데
	 * RestTemplate은 VO를 감싸는 List가 필요하다
	 * 그런데 List를 감싸는 VO가 News, Books, Movies 들이 각가 다르다
	 * 이럴때 Parent 클래스를 각가의 VO를 감싸는 타입으로 복수 정의해야 하는 문제가 있다.
	 * 
	 * 이때도 Generic의 기능을 사용하여 
	 * 클래스 선언문에 Generic을 정의해 두고
	 * 이 클래스를 사용할 때 확정된 VO를 부착하여 사용한다
	 */
